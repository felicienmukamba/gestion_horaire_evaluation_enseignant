...
class PDF(FPDF):
    def __init__(
        ...
        **kwargs,
        ...
        last_horaire = self.horaires.objects.filter(
            anneeAcademique=self.anneeAcademique
        )[0]
    ...
    def header(self):
        ...
        self.cell(
            0,
            5,
            f"HORAIRE DES COURS : SEMAINE DU LUNDI {format(self.lundi)} AU SAMEDI {format(self.samedi)} VACATION {self.vacation.upper()}",
            ln=1,
            align="C",
        )
        self.ln(7)
        ...
        for promotion in self.promotions:
            for departement in self.departements:
                ...
                self.cell(
                    32,
                    12,
                    f"{promotion.designation} {departement_sigle}",
                    border=1,
                    align="C",
                )
                lundi = self.horaires.objects.filter(
                    date=self.lundi,
                    anneeAcademique=self.anneeAcademique,
                ...
                if lundi.exists():
                    ...
                    chargeHoraire = self.chargesHoraires.objects.filter(
                        cours=cours.id,
                        anneeAcademique=self.anneeAcademique,
                        promotion=promotion,
                        departement=departement,
                    )
                    enseignant = self.enseignants.objects.get(
                        id=chargeHoraire[0].enseignant.id
                    )
                    salle = self.salles.objects.get(id=lundi[0].salle.id)
                    enseignant_noms = (
                        enseignant.titreAcademique
                        + " "
                        + enseignant.prenom
                        + " "
                        + enseignant.nom
                        if enseignant.prenom
                        else enseignant.titreAcademique
                        + " "
                        + enseignant.nom
                        + " "
                        + enseignant.postnom
                ...
                mardi = self.horaires.objects.filter(
                    date=self.mardi,
                    anneeAcademique=self.anneeAcademique,
                    vacation=self.vacation,
                    departement=departement,
                ...
                mercredi = self.horaires.objects.filter(
                    date=self.mercredi,
                    anneeAcademique=self.anneeAcademique,
                    vacation=self.vacation,
                    departement=departement,
                ...
                jeudi = self.horaires.objects.filter(
                    date=self.jeudi,
                    anneeAcademique=self.anneeAcademique,
                    vacation=self.vacation,
                    departement=departement,
                ...
                vendredi = self.horaires.objects.filter(
                    date=self.vendredi,
                    anneeAcademique=self.anneeAcademique,
                    vacation=self.vacation,
                    departement=departement,
                ...
                samedi = self.horaires.objects.filter(
                    date=self.samedi,
                    anneeAcademique=self.anneeAcademique,
                    vacation=self.vacation,
                    departement=departement,
